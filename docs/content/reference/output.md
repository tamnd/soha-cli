...
describes what users get. Pick a format with `-o`, or let soha choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
soha <command> -o table   # aligned columns for reading
soha <command> -o jsonl   # one JSON object per line, for piping
soha <command> -o json    # a single JSON array
soha <command> -o csv     # spreadsheet friendly
soha <command> -o tsv     # tab-separated
soha <command> -o url     # just the URL column
soha <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
soha <command> --fields id,title,url
```
...
```bash
soha <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
soha <command>            # a table, because this is a terminal
soha <command> | wc -l    # JSONL, because this is a pipe
```
